/**
 * inviteAgent.service.ts
 * Service for creating agent invitations with validation and authorization
 */
...
import type { InviteAgentResponse } from './inviteAgent.dto.js'
import type { InviteAgentInput } from './inviteAgent.schema.js'
 ...
 *
 * Authorization:
 * - Only BROKER_EMPLOYEES (SUPER_ADMIN, CLAIMS_EMPLOYEE, OPERATIONS_EMPLOYEE, ADMIN_EMPLOYEE) can invite agents
 * - CLIENT_ADMIN and AFFILIATE cannot invite agents
 *
 * Validation:
...
export async function inviteAgent(
  ...
  if (!roleName || !BROKER_EMPLOYEES.includes(roleName as never)) {
    logger.warn({ userId, role: roleName }, 'Unauthorized agent invitation attempt')
    throw new ForbiddenError('No tienes permiso para invitar agentes')
  }
  ...
  // STEP 8: Prepare Entity Data (stored for agent creation on acceptance)
  const entityData = {
  ...
  const response: InviteAgentResponse = {
    id: invitation.id,
    email: invitation.email,
