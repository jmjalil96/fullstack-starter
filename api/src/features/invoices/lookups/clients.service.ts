/**
 * clients.service.ts
 * Service for fetching available clients for invoice creation
 */
...
export async function getAvailableClients(userId: string): Promise<AvailableClientResponse[]> {
  ...
  // STEP 4: Log Activity
  logger.info(
    { userId, role: roleName, count: clients.length },
    'Broker employee accessed clients for invoice creation'
  )
