/**
 * clients.service.ts
 * Service for fetching available clients for claim creation
 */
...
export async function getAvailableClients(userId: string): Promise<AvailableClientResponse[]> {
  ...
  if (isBrokerEmployee) {
    ...
    logger.info({ userId, role: roleName, count: clients.length }, 'Broker employee accessed clients for claims')
    ...
    logger.info({ userId, clientIds: accessibleClientIds, count: clients.length }, 'CLIENT_ADMIN accessed clients for claims')
  } else if (isAffiliate) {
    ...
    logger.info({ userId, clientId: user.affiliate.clientId, found: !!client }, 'AFFILIATE accessed their client for claims')
  }
